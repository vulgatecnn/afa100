...
describe('MySQL数据库配置测试', () => {
  ...
  it('应该根据性能指标调整配置', () => {
    ...
    const adjustments = adjustDatabaseConfig(currentConfig, performanceMetrics);
    
    expect(adjustments).toBeDefined();
    if (adjustments.performance) {
      expect(adjustments.performance.slowQueryThreshold).toBeGreaterThan(currentConfig.performance.slowQueryThreshold);
    }
  });

  it('应该处理不同环境的配置', () => {
    const devConfig = getOptimizedDatabaseConfig('development');
    const testConfig = getOptimizedDatabaseConfig('test');
    const prodConfig = getOptimizedDatabaseConfig('production');
    
    expect(devConfig.performance.enableQueryLogging).toBe(true);
    expect(testConfig.connectionLimit).toBeLessThanOrEqual(5);
    expect(prodConfig.performance.enableQueryLogging).toBe(false);
  });
