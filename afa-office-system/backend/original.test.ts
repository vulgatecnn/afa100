...
describe('SpaceController', () => {
    ...
    let mockSpaceService: vi.Mocked<SpaceService>;
    ...
    describe('getSpaceHierarchy', () => {
        it('应该成功获取空间层级结构', async () => {
            ...
            // Act
            await spaceController.getSpaceHierarchy(mockRequest as Request, mockResponse as Response);
    ...
    describe('getVenues', () => {
        it('应该成功获取场地列表', async () => {
            ...
            // Act
            await spaceController.getVenues(mockRequest as Request, mockResponse as Response);
    ...
    describe('updateVenue', () => {
        it('应该成功更新场地', async () => {
            // Arrange
            const venueId = '1';
            const updateData = { name: '更新后的场地名' };
            const updatedVenue = { id: 1, name: '更新后的场地名', address: '原地址' };
            ...
            // Act
            await spaceController.updateVenue(mockRequest as Request, mockResponse as Response);

            // Assert
            expect(mockSpaceService.updateVenue).toHaveBeenCalledWith(parseInt(venueId), updateData);
            expect(mockResponse.json).toHaveBeenCalledWith({
                success: true,
                data: updatedVenue,
                message: '更新场地成功'
        ...
        it('应该处理场地不存在的情况', async () => {
            ...
            mockRequest.params = { id: venueId };
            mockRequest.body = { name: '新名称' };
            mockSpaceService.updateVenue.mockRejectedValue(error);
    ...
    describe('deleteVenue', () => {
        it('应该成功删除场地', async () => {
            ...
            // Act
            await spaceController.deleteVenue(mockRequest as Request, mockResponse as Response);
    ...
    describe('getFloors', () => {
        it('应该成功获取楼层列表', async () => {
            ...
            mockSpaceService.getFloors.mockResolvedValue(mockFloors);

            // Act
            await spaceController.getFloors(mockRequest as Request, mockResponse as Response);

            // Assert
            expect(mockSpaceService.getFloors).toHaveBeenCalledWith(parseInt(venueId));
            expect(mockResponse.json).toHaveBeenCalledWith({
                success: true,
                data: mockFloors,
                message: '获取楼层列表成功'
    ...
    describe('createFloor', () => {
        it('应该成功创建楼层', async () => {
            ...
            // Act
            await spaceController.createFloor(mockRequest as Request, mockResponse as Response);
    ...
    describe('getProjects', () => {
        it('应该成功获取项目列表', async () => {
            ...
            const floorId = '1';
            const mockProjects = [
                { id: 1, name: '项目A', floorId: 1 },
                { id: 2, name: '项目B', floorId: 1 }
            ];

            mockRequest.params = { floorId };
            mockSpaceService.getProjects.mockResolvedValue(mockProjects);

            // Act
            await spaceController.getProjects(mockRequest as Request, mockResponse as Response);

            // Assert
            expect(mockSpaceService.getProjects).toHaveBeenCalledWith(parseInt(floorId));
            expect(mockResponse.json).toHaveBeenCalledWith({
                success: true,
                data: mockProjects,
                message: '获取项目列表成功'
    ...
    describe('createProject', () => {
        it('应该成功创建项目', async () => {
            ...
            const createdProject = { id: 1, floorId: 1, ...projectData };
            ...
            // Act
            await spaceController.createProject(mockRequest as Request, mockResponse as Response);

            // Assert
            expect(mockSpaceService.createProject).toHaveBeenCalledWith(parseInt(floorId), projectData);
            expect(mockResponse.status).toHaveBeenCalledWith(201);
            expect(mockResponse.json).toHaveBeenCalledWith({
                success: true,
                data: createdProject,
